test_that("Observation Period Overlap Query", {
  ...
  query <- SqlRender::render(query, cdmDatabaseSchema = "main")
  query <- SqlRender::translate(query, targetDialect = "duckdb")
  ...
  # 5 not expected in results because the two obs periods have no overlap
  # 6 not expected in results because has only one observation period
  testthat::expect_equal(result[result$person_id == 1, 'n_overlapping_pairs'], 3, info = "person_id 1 should have 3 overlapping pairs")
  testthat::expect_equal(result[result$person_id == 2, 'n_overlapping_pairs'], 3, info = "person_id 2 should have 3 overlapping pairs")
  testthat::expect_equal(result[result$person_id == 3, 'n_overlapping_pairs'], 1, info = "person_id 3 should have 1 overlapping pairs")
  testthat::expect_equal(result[result$person_id == 4, 'n_overlapping_pairs'], 2, info = "person_id 4 should have 2 overlapping pairs")
  testthat::expect_equal(nrow(result[result$person_id == 5, ]), 0, info = "person_id 5 should have no overlapping pairs")
  testthat::expect_equal(nrow(result[result$person_id == 6, ]), 0, info = "person_id 6 should have no overlapping pairs")
